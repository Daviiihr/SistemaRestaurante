...
class AplicacionConPestanas(ctk.CTk):
    def __init__(self):
        ...
        # TAREA 1: INICIALIZAR EL ATRIBUTO EN __init__
        self.tarjetas_frame = None 
    ...
    def cargar_csv(self):
        try:
            ...
            if archivo_csv:  
                self.df_csv = pd.read_csv(archivo_csv)
                ...
                CTkMessagebox(title="Éxito", message=f"CSV cargado correctamente!", icon="check")
        except Exception as e:
            CTkMessagebox(title="Error", message=f"Error al cargar el archivo CSV: {str(e)}", icon="cancel")
    ...
    def generar_y_mostrar_carta_pdf(self):
        try:
            ...
            if self.pdf_viewer_carta is not None:
                try:
                    self.pdf_viewer_carta.pack_forget(); self.pdf_viewer_carta.destroy()
    ...
    def mostrar_boleta(self, pdf_path): # Modificar
        try:
            if self.pdf_viewer_boleta: self.pdf_viewer_boleta.destroy()
            abs_path = os.path.abspath(pdf_path)
            self.pdf_viewer_boleta = CTkPDFViewer(self.pdf_frame_boleta, file=abs_path)
            self.pdf_viewer_boleta.pack(expand=True, fill="both")
        except Exception as e:
            CTkMessagebox(title="Error", message=f"No se pudo mostrar el PDF de la boleta.\n{e}", icon="warning")
    ...
    def generar_menus(self):
        # TAREA 1: USAR self.tarjetas_frame
    ...
    # TAREA 2: IMPLEMENTAR LA FUNCIÓN eliminar_menu
    def eliminar_menu(self):
        seleccion = self.treeview_menu.selection()
        if not seleccion:
            CTkMessagebox(title="Aviso", message="Selecciona un item del pedido para eliminar.", icon="warning")
            return
    ...
    def generar_boleta(self):
        if not self.pedido.menus:
            CTkMessagebox(title="Pedido Vacío", message="No hay elementos en el pedido para generar una boleta", icon="warning")
            return

        try:
            # 1. Crear la boleta usando el Facade
            boleta_facade = BoletaFacade(self.pedido)
            pdf_path = boleta_facade.generar_boleta()  # Este método debe retornar la ruta del archivo
            
            # 2. Mostrar un mensaje de éxito
            CTkMessagebox(title="Éxito", message=f"Boleta generada correctamente en:\n{pdf_path}", icon="check")
            
            # 3. Cambiar a la pestaña "Boleta" y mostrar el PDF
            self.tabview.set("Boleta")
            self.mostrar_boleta(pdf_path)

        except Exception as e:
            CTkMessagebox(title="Error", message=f"No se pudo generar la boleta.\n{e}", icon="warning")

    def configurar_pestana2(self):
        frame_superior = ctk.CTkFrame(self.tab2); frame_superior.pack(side="top", fill="both", expand=True, padx=10, pady=10)
        frame_intermedio = ctk.CTkFrame(self.tab2); frame_intermedio.pack(side="top", fill="x", padx=10, pady=5)
        
        # TAREA 1: ELIMINAR global Y USAR self.tarjetas_frame
    ...
    def crear_tarjeta(self, menu):
        ...
        # TAREA 1: USAR self.tarjetas_frame
        tarjeta = ctk.CTkFrame(self.tarjetas_frame, corner_radius=10, border_width=1, border_color="#4CAF50", width=64, height=140, fg_color="gray")
    ...
    def validar_nombre(self, nombre):
        if re.match(r"^[a-zA-Z\s]+$", nombre): return True
        else: CTkMessagebox(title="Error de Validación", message="El nombre debe contener solo letras y espacios.", icon="warning"); return False

    def validar_cantidad(self, cantidad):
        if cantidad.isdigit(): return True
        else: CTkMessagebox(title="Error de Validación", message="La cantidad debe ser un número entero positivo.", icon="warning"); return False

    def ingresar_ingrediente(self):
        nombre, unidad, cantidad = self.entry_nombre.get().strip(), self.combo_unidad.get().strip() or None, self.entry_cantidad.get().strip()
        if not nombre or not cantidad:
            CTkMessagebox(title="Error", message="Completa todos los campos.", icon="warning"); return
        if not self.validar_nombre(nombre) or not self.validar_cantidad(cantidad): return
        ingrediente = Ingrediente(nombre=nombre, unidad=unidad, cantidad=float(cantidad))
        self.stock.agregar_ingrediente(ingrediente); self.actualizar_treeview()
        self.entry_nombre.delete(0, "end"); self.entry_cantidad.delete(0, "end")

    def eliminar_ingrediente(self):
        if not (seleccion := self.tree.selection()):
            CTkMessagebox(title="Aviso", message="Selecciona un ingrediente para eliminar.", icon="warning"); return
        item_id = seleccion[0]
        valores = self.tree.item(item_id, "values")
        nombre, unidad = valores[0], valores[1] if len(valores) > 1 else None
        if not self.stock.eliminar_ingrediente(nombre, None if unidad in (None, "-", "") else unidad):
            CTkMessagebox(title="Aviso", message="No se pudo eliminar el ingrediente seleccionado.", icon="warning"); return
        self.actualizar_treeview()
    ...
    def actualizar_treeview(self):
        ...
        for ingrediente in self.stock.lista_ingredientes:
            self.tree.insert("", "end", values=(ingrediente.nombre, ingrediente.unidad or "-", ingrediente.cantidad))
